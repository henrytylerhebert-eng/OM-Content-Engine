...
import src.reporting.raw_pipeline as raw_pipeline
from src.ingest.airtable_import import build_raw_import_records
from src.ingest.airtable_live import AirtableReadConfig
from src.reporting.raw_pipeline import build_local_bundle, resolve_source_paths
from tests.fixtures.pilot_rows import ACTIVE_MEMBER_ROWS, COHORT_EXPORT_ROWS, MENTOR_ROWS
...
def test_build_local_bundle_can_read_live_airtable_source(monkeypatch: pytest.MonkeyPatch) -> None:
    table_rows = {
        "Active Members": build_raw_import_records(
            ACTIVE_MEMBER_ROWS,
            source_table="Active Members",
            source_system="airtable_live",
            file_path="airtable://appExample123/Active Members",
        ),
        "Mentors": build_raw_import_records(
            MENTOR_ROWS,
            source_table="Mentors",
            source_system="airtable_live",
            file_path="airtable://appExample123/Mentors",
        ),
        "Cohorts": build_raw_import_records(
            COHORT_EXPORT_ROWS,
            source_table="Cohorts",
            source_system="airtable_live",
            file_path="airtable://appExample123/Cohorts",
        ),
    }

    monkeypatch.setattr(
        raw_pipeline.AirtableReadConfig,
        "from_env",
        classmethod(lambda cls: AirtableReadConfig(token="test-token", base_id="appExample123")),
    )

    class FakeReadClient:
        def __init__(self, config: AirtableReadConfig) -> None:
            self.config = config

    monkeypatch.setattr(raw_pipeline, "AirtableReadClient", FakeReadClient)
    monkeypatch.setattr(
        raw_pipeline,
        "load_airtable_live_records",
        lambda client, *, table_name, required=True: list(table_rows.get(table_name, [])),
    )

    bundle = build_local_bundle(source="airtable")

    assert bundle["raw_sources"]["active_members"]["file_path"] == "airtable://appExample123/Active Members"
    assert bundle["raw_sources"]["mentors"]["file_path"] == "airtable://appExample123/Mentors"
    assert bundle["raw_sources"]["cohorts"]["file_path"] == "airtable://appExample123/Cohorts"
    assert bundle["raw_sources"]["active_members"]["row_count"] == len(ACTIVE_MEMBER_ROWS)
    assert bundle["raw_sources"]["mentors"]["row_count"] == len(MENTOR_ROWS)
    assert bundle["raw_sources"]["cohorts"]["row_count"] == len(COHORT_EXPORT_ROWS)
    assert bundle["ecosystem_summary"]["organization_count"] == 6
    assert bundle["ecosystem_summary"]["participation_count"] == 4
